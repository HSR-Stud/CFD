...
\colorbox{red}{Aus anderem CFD Buch übernehmen!!!}
...
\begin{align}
\boxed{\Gamma A \phi'' \frac{1}{2} \left(\delta x_{PE} + \delta
x_{WP}\right)+\Gamma A \phi''' \frac{1}{6}\left[\left(\delta
x_{PE}\right)^2-\left(\delta \x_{WP}\right)^2\right]}
\end{align}
...
\subsection{Multi-Grid Methods} Describe the idea behind the multi-grid method
Anfangsbedingungne 
...
Unsicherheiten) limitierte genauigkeit
...
validation mean.
\begin{itemize}
\item verification, mathematisch teil, richtiges lösen der Gleichungen
\item validation, ingenieur, muss die richtigen Gleichungen finden, überprüfen und Beschreibung, so dass es der Wirklichkeit entspricht.
\end{itemize}
